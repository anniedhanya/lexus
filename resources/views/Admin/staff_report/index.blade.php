                                  ...
                                  <div class="card card-body mt-3 filter_card">
                                    ...
                                    <form id="formFilterNew" method="post" action="{{url('/')}}">
                                      <div class="row">
                                          ...
                                          {!!Form::select('search_assigned_user_id', App\Models\User::listForSelectAssigned('Select', 10000),null, array('class'=>'itemName form-control  ', 'id'=>'search_assigned_user_id')) !!} 
                                          </div>
                                         
                                        <div class="col-lg-4 mb-2">
                                          <label for="fullname">Assigned From Date </label>
                                         ...
                                         <div class="col-lg-4 mb-2">
                                          <label for="fullname">Assigned To Date </label>
...
// </script>
  ...
  $("#toggle_status").change(function () {
     ...
     $("#per_page_count").val($("#per_page").val());
          ...
          var page_number = 1;
     $('#datatable_ tbody').html('<tr><th colspan="13">Loading <i class="fa fa-spinner fa-spin"></i></th></tr>');
          $.ajaxSetup({
              headers: {
          ...
          $.ajax({
              url:"{{url('/')}}/admin/staff_report/staff_report_list",
                ...
                { $('#datatable_ tbody').html('');
                  if(data.total_count > 0)
                  {
                      ...
                      for(var i=0;i<data.count;i++)
                      {
                        ...
                        str += '<td>'+slno+'</td><td>'+data.result[i].id+'</td><td>'+data.result[i].name+'</td><td>'+data.result[i].contact_no+'</td><td>'+data.result[i].order_id+'</td><td>'+data.result[i].category+'</td><td>'+data.result[i].executive+'</td><td>'+data.result[i].warranty_status+'</td><td>'+data.result[i].assignee+'</td><td>'+data.result[i].assigned_date+'</td><td>'+data.result[i].screenshot_status+'</td><td>'+data.result[i].remarks+'</td><td>'+data.result[i].created_on+'</td>'; 
                  ...
                  {
                    $('#datatable_ tbody').html('<tr><th colspan="13">No Data Available</th></tr>');
                  }
                }
                else
                {
                  ...
                  $('#datatable_ tbody').html('<tr><th colspan="13">Something went wrong! Please try again.</th></tr>');
    ...
    var baseurl = $('#baseurl').val();
         ...
         $("#export_csv").click(function(){
          ...
          var search = $('#search').val();
            ...
            window.location.href =  '{{url('/')}}/admin/staff_report_export?assigned_to='+search_assigned_user_id+'&warranty_period_from='+warranty_period_from+'&warranty_period_to='+warranty_period_to+'&search='+search_val;
