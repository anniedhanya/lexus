...
trait ValidationTrait
{
        /**
    * ILEAF
    * This model is used to set default functions
    ...
    */
    ...
    public function validate($data = null, $throw_rror = true) {
        ...
        if ($validator->fails()) {
            ...
            if($throw_rror) {
                if (Request::ajax())
                    $response = new JsonResponse(['success' => false, 'errors' => $this->errors], 422);
                else
                    $response = Redirect::back()->withInput()->withErrors($this->errors->toArray());
